...
import { getTodayForCountry, buildISODateFromString } from '@/lib/dateUtils';
...
export default function DashboardPage() {
  ...
  const { user, supabaseTransactions, addTransaction, getTodayMovements, getTodayDeletedCount, updateUser, loading } = useSupabase();
  ...
  useEffect(() => {
    if (user) {
      ...
      // has_seen_onboarding = false o undefined = mostrar tutorial
      // has_seen_onboarding = true = no mostrar tutorial
      if (user.has_seen_onboarding === false || user.has_seen_onboarding === undefined) {
        setShowOnboarding(true);
      }
    }
  }, [user]);

  // Cargar contador de transacciones eliminadas HOY
  useEffect(() => {
    const loadDeletedCount = async () => {
      if (user) {
        const count = await getTodayDeletedCount();
        setDeletedCount(count);
      }
    };
    loadDeletedCount();
  }, [user, getTodayDeletedCount, supabaseTransactions]); // Recargar cuando cambien las transacciones
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex-1">
              <p className="text-lg font-bold text-gray-900">
                {todayMovements.length} <span className="text-xs font-normal text-gray-600">Transacciones</span>
              </p>
              {deletedCount > 0 && (
