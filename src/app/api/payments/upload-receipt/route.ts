...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Validar tipo MIME y contenido (magic bytes)
    const fileValidation = await validateFile(file, allowedTypes);
    if (!fileValidation.valid) {
      logger.warn('⚠️ Intento de subir archivo inválido:', {
        fileName: file.name,
        fileType: file.type,
      ...
      return NextResponse.json(
        { error: fileValidation.error || 'Tipo de archivo no permitido. Solo se aceptan JPG, PNG o PDF' },
        { status: 400 }
      );
    }
