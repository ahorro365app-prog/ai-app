...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      return handleError(error, 'Error obteniendo configuraciones de versiones');
    }
    ...
    return handleError(error, 'Error obteniendo versiones');
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (checkError && checkError.code !== 'PGRST116') { // PGRST116 = no rows returned
      return handleError(checkError, 'Error verificando configuración existente');
    }
    ...
    if (existing) {
      ...
      if (deactivateError) {
        return handleError(deactivateError, 'Error desactivando configuración anterior');
      }
    }
    ...
    if (insertError) {
      return handleError(insertError, 'Error creando nueva configuración');
    }
    ...
    return handleError(error, 'Error actualizando versión');
  }
}
