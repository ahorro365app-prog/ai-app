...
export async function POST(request: NextRequest) {
  ...
  if (error) {
    return handleError(error, 'No se pudieron obtener campañas programadas')
  }
  ...
  try {
    ...
    logger.error('⚠️ Error activando Smart programado:', error)
  ...
  try {
    ...
    logger.warn('⚠️ No se pudo obtener health check anterior:', err?.message)
  }
  ...
  try {
    ...
    if (healthError) {
      logger.error('⚠️ Error guardando health check del cron:', healthError)
      // No fallar el endpoint si el health check falla
    } else {
      logger.debug('✅ Health check del cron guardado exitosamente')
    }
  } catch (healthErr: any) {
    logger.error('⚠️ Error guardando health check del cron:', healthErr)
    // No fallar el endpoint si el health check falla
  }
  ...
  try {
    ...
    if (alertsResult.errors.length > 0) {
      logger.warn('⚠️ Errores enviando alertas:', alertsResult.errors)
    }
  } catch (alertErr: any) {
    logger.error('⚠️ Error verificando/enviando alertas:', alertErr)
