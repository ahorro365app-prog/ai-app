...
async function countByStatus(status: string) {
  ...
  if (error) {
    logger.error(`Error contando status ${status}:`, error);
    throw error;
  }
...
async function countColumnNotNull(column: string): Promise<CountResult> {
  ...
  if (response.error) {
    ...
    logger.error(`Error contando columna ${column}:`, response.error);
...
async function countStatusWithinRange(status: string, fromIso: string) {
  ...
  if (error) {
    logger.error(`Error contando status ${status} en rango`, error);
    throw error;
  }
...
async function countColumnNotNullWithinRange(column: string, fromIso: string) {
  ...
  if (response.error) {
    ...
    logger.error(`Error contando columna ${column} en rango:`, response.error);
    throw response.error;
  }
...
export async function GET(_request: NextRequest) {
  try {
    ...
    if (totalError) {
      return handleError(totalError, 'No se pudo obtener el resumen');
    }
    ...
    for (const [key, fromIso] of Object.entries(rangeMap) as Array<[TimeWindowKey, string]>) {
      try {
        ...
        logger.error(`Error calculando métricas para ${key}:`, error);
    ...
    if (recentLogsError) {
      logger.error('Error obteniendo logs recientes para agregaciones:', recentLogsError);
    } else if (recentLogs) {
    ...
    try {
      ...
      if (triggerLogsError) {
        logger.error('Error obteniendo últimos trigger logs:', triggerLogsError);
      } else if (triggerLogs) {
      ...
      logger.error('Error consultando notification_trigger_logs:', triggerError);
    }
    ...
    try {
      ...
      logger.error('Error obteniendo health check del cron:', healthErr);
    ...
    return handleError(error, 'Error obteniendo resumen de logs');
  }
}
