...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const errorType = !data ? ErrorType.NOT_FOUND : undefined;
    return handleError(error, 'Error obteniendo template', errorType);
  }
}
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return handleError(error, 'Error actualizando template');
  }
}
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return handleError(error, 'Error eliminando template');
  }
}
