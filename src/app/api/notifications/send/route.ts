...
export async function POST(request: NextRequest) {
  try {
    ...
    if (targetType === 'token') {
      ...
      if (!isPreview) {
        ...
        if (prefError && prefError.code !== 'PGRST116') {
          logger.error('Error obteniendo preferencias de notificación del usuario:', prefError);
          return NextResponse.json(
      ...
      try {
        ...
        logger.error('Error resolviendo segmento de notificaciones:', segmentError);
        return NextResponse.json(
          {
            success: false,
    ...
    logger.error('Error en /api/notifications/send:', error);
    return NextResponse.json(
