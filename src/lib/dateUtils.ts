...
const COUNTRY_NAME_TO_CODE: Record<string, SupportedCountry> = {
  'BOLIVIA': 'BO',
  'ARGENTINA': 'AR',
  'BRASIL': 'BR',
  'BRAZIL': 'BR',
...
export const getTimezoneForCountry = (countryCode?: string) => {
  ...
  if (normalized.length === 2) {
    return COUNTRY_TIMEZONE[normalized as SupportedCountry] || DEFAULT_TIMEZONE;
  }
  
  // Si es un nombre de país, buscar en el mapeo
  if (COUNTRY_NAME_TO_CODE[normalized]) {
    const code = COUNTRY_NAME_TO_CODE[normalized];
    return COUNTRY_TIMEZONE[code] || DEFAULT_TIMEZONE;
  }
  
  // Fallback: intentar usar las primeras 2 letras
  const firstTwo = normalized.substring(0, 2) as SupportedCountry;
  if (COUNTRY_TIMEZONE[firstTwo]) {
    return COUNTRY_TIMEZONE[firstTwo];
  }
  
  // Último fallback: timezone por defecto
  logger.warn(`⚠️ País no reconocido: "${countryCode}", usando timezone por defecto`);
...
export const getTodayForCountry = (countryCode?: string): string => {
  ...
  // SOLUCIÓN ROBUSTA: Usar Intl.DateTimeFormat con formatToParts
  // Esto es más confiable que toLocaleString para parsing
  const formatter = new Intl.DateTimeFormat('en-US', {
    timeZone,
    year: 'numeric',
    month: '2-digit',
 ...
 * 
 * @example
 * extractDateInUserTimezone('2025-11-26T00:38:22+00:00', 'BO')
 * // Retorna: '2025-11-25' (porque 00:38 UTC = 20:38 del día anterior en UTC-4)
 */
export const extractDateInUserTimezone = (
  ...
  if (isNaN(date.getTime())) {
    ...
    try {
      ...
      logger.error('❌ Error en fallback de parseo de fecha:', e);
    }
    logger.warn('⚠️ Usando fecha actual como fallback');
  ...
  // 5. SOLUCIÓN SIMPLIFICADA: Usar toLocaleString para obtener fecha en zona horaria del país
  // Esto es más confiable que Intl.DateTimeFormat para extraer solo el día
